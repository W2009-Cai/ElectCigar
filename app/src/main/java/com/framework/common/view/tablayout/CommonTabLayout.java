...
import com.framework.common.view.tablayout.utils.FragmentChangeManager;
import com.framework.common.view.tablayout.utils.UnreadMsgUtils;
...
public class CommonTabLayout extends FrameLayout implements ValueAnimator.AnimatorUpdateListener {
    ...
    public void setCanExchange(boolean flag){
        this.canExchange=flag;
    }
    /***是否单行显示,默认单行**/
    private boolean isSingleLine=true;

    public void setSingleLine(boolean flag){
        this.isSingleLine = flag;
    }
...
//        Log.d("AAA", "mCurrentP--->" + mCurrentP.left + "&" + mCurrentP.right);
    ...
    public void showMsg(int position, int num) {
        ...
        tipView.setBackgroundColor(getResources().getColor(R.color.main_color));
        if (tipView != null) {
    ...
    public void showRedDot(int position){
        ...
        View tabView = mTabsContainer.getChildAt(position);
        View dot = tabView.findViewById(R.id.red_dot);
        if (dot!=null) {
    ...
    public void setTabText(CharSequence text, int position) {
        if (position >= mTabCount) {
            position = mTabCount - 1;
        }
        View tabView = mTabsContainer.getChildAt(position);
