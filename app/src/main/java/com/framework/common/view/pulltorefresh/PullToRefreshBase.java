...
import com.framework.common.view.pulltorefresh.internal.FlipLoadingLayout;
import com.framework.common.view.pulltorefresh.internal.LoadingLayout;
import com.framework.common.view.pulltorefresh.internal.RotateLoadingLayout;
import com.framework.common.view.pulltorefresh.internal.Utils;
import com.framework.common.view.pulltorefresh.internal.ViewCompat;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
	...
	static final float FRICTION = 2.0f;									// 上拉和下拉的最大距离 1/2
	...
	public void addView(View child, int index, ViewGroup.LayoutParams params) {
		...
		ILog.d(TAG, "addView: " + child.getClass().getSimpleName());
	...
	public void setNoMoreData(final boolean mNoMoreData) {
		this.mNoMoreData = mNoMoreData;
		if (mFooterLayout != null) {
			int delay = mNoMoreData ? 300 : 0;
			MainHandler.postDelayed(new Runnable() {
				
				@Override
				public void run() {
					//TODO set nomore
					mFooterLayout.setNoMoreData(mNoMoreData);
				}
			}, delay);
		}
	}
	...
	@Override
	public void onRefreshComplete(int delayed) {
		MainHandler.postDelayed(new Runnable() {
			@Override
			public void run() {
				if (isRefreshing()) {
					...
					String time = IDateFormatUtil.getFormatTime(lastRefreshTime, IDateFormatUtil.HH_MM_SS);
					if (getHeaderLayout() != null) {
						getHeaderLayout().setLastUpdatedLabel(time);
					}
					if (getHeaderLoadingView() != null) {
						getHeaderLoadingView().setLastUpdatedLabel(time);
					}
				}
			}
		}, delayed);
	...
	/**
	 * 重新设置起点(down)的坐标,避免中断事件传递,滑动一段距离之后,重新处理事件导致坐标相减得到的位移不正确
	 * @param x
	 * @param y
     */
	public void resetDownPosition(int x, int y) {
		mInitialMotionX = x;
		mInitialMotionY = y;
	}
		...
		switch (event.getAction()) {
			...
			case MotionEvent.ACTION_UP: {
				if (mIsBeingDragged) {
					...
					//TODO 没有更多时，停留一段时间
					if (mCurrentMode == Mode.PULL_FROM_END && mNoMoreData) {
	...
	public final void setMode(Mode mode) {
		if (mode != mMode) {
			ILog.d(TAG, "Setting mode to: " + mode);
	...
	public final void removePullFromEndMode() {
		switch (mMode) {
			case PULL_FROM_END:
	...
	public final void removePullFromStartMode() {
		switch (mMode) {
	...
	public final void addPullFromEndMode() {
		switch (mMode) {
			case DISABLED:
				setMode(Mode.PULL_FROM_END);
				break;
	...
	public final void addPullFromStartMode() {
		switch (mMode) {
			case DISABLED:
				setMode(Mode.PULL_FROM_START);
				break;
	...
	private void onlyShowLoading(){
		if (!isRefreshing()) {
			...
			int headerSize=getHeaderSize();
			ILog.i("PullToRefreshBase","---showLoading  size="+headerSize);
			smoothScrollTo(-headerSize);
		}
	}
	...
	public void showRefresh(int delayed) {
		// 需要延迟加载，否则视图还没加载出来，头视图还没有大小
		MainHandler.postDelayed(new Runnable() {
			@Override
			public void run() {
				if (!isRefreshing()) {
...
//						listview.smoothScrollToPosition(0);
	...
	protected final void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		//ILog.d(TAG, String.format("onSizeChanged. W: %d, H: %d", w, h));
	...
	protected final void refreshLoadingViewsSize() {
		...
		//ILog.d(TAG, String.format("Setting Padding. L: %d, T: %d, R: %d, B: %d", pLeft, pTop, pRight, pBottom));
	...
	protected final void setHeaderScroll(int value) {
		...
		//ILog.d(TAG, "setHeaderScroll: " + value);
