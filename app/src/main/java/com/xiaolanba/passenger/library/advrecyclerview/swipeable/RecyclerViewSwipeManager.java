...
package com.xiaolanba.passenger.library.advrecyclerview.swipeable;
...
import com.xiaolanba.passenger.library.advrecyclerview.animator.SwipeDismissItemAnimator;
import com.xiaolanba.passenger.library.advrecyclerview.swipeable.action.SwipeResultAction;
import com.xiaolanba.passenger.library.advrecyclerview.swipeable.action.SwipeResultActionDefault;
import com.xiaolanba.passenger.library.advrecyclerview.utils.CustomRecyclerViewUtils;
import com.xiaolanba.passenger.library.advrecyclerview.utils.WrapperAdapterUtils;

/**
 * Provides item swipe operation for {@link RecyclerView}
 */
@SuppressWarnings("PointlessBitwiseExpression")
public class RecyclerViewSwipeManager implements com.xiaolanba.passenger.library.advrecyclerview.swipeable.SwipeableItemConstants {
    ...
    private static final int MIN_DISTANCE_TOUCH_SLOP_MUL = /*10*/8;
    ...
    private boolean handleActionDown(RecyclerView rv, MotionEvent e) {
        ...
        if (!(holder instanceof com.xiaolanba.passenger.library.advrecyclerview.swipeable.SwipeableItemViewHolder)) {
            return false;
        }
    ...
    private void handleActionUpOrCancelWhileSwiping(int action) {
        ...
        if (action == MotionEvent.ACTION_UP) {
            ...
            Log.d(TAG, "absDistance = " + absDistance);
            Log.d(TAG, "mTouchSlop * MIN_DISTANCE_TOUCH_SLOP_MUL = " + mTouchSlop * MIN_DISTANCE_TOUCH_SLOP_MUL);
            Log.d(TAG, "(distance * velocity) > 0.0f = " + ((distance * velocity) > 0.0f));
            Log.d(TAG, "absVelocity <= mMaxFlingVelocity = " + (absVelocity <= mMaxFlingVelocity));
            Log.d(TAG, "(absDistance > (viewSize / 2) = " + ((absDistance > (viewSize / 5))));
            Log.d(TAG, "absVelocity >= mMinFlingVelocity = " + ((absVelocity >= mMinFlingVelocity)));
            Log.d(TAG, "mSwipingItemReactionType = " + mSwipingItemReactionType);
            ...
            if ((absDistance > (mTouchSlop * MIN_DISTANCE_TOUCH_SLOP_MUL)) &&
                    ...
                    ((absDistance > (viewSize / 5)) || (absVelocity >= mMinFlingVelocity))) {
    ...
    public void removeItemAction(RecyclerView.ViewHolder swipingItem,int position,int result){
        if (mItemSlideAnimator != null) {
            mItemSlideAnimator.finishSwipeSlideToOutsideOfWindow(swipingItem, -1, true, 0, position, null);
        }
    }
    ...
    /*package*/ void applySlideItem(
        ...
        final com.xiaolanba.passenger.library.advrecyclerview.swipeable.SwipeableItemViewHolder holder2 = (com.xiaolanba.passenger.library.advrecyclerview.swipeable.SwipeableItemViewHolder) holder;
